 ...
 */
const { admin, firestoreAdmin } = require('../../config/firebase');
...
const isPlaylistManageable = async (userId, playlistName) => {
  try {
    console.log(`[PlaylistRepository] Checking management rights for user ${userId} on playlist "${playlistName}"`);
    ...
    for (const userPlaylists of allPlaylists) {
      ...
      if (foundPlaylist) {
        ...
        console.log(`[PlaylistRepository] Playlist "${playlistName}" found, owned by ${foundPlaylist.author}, requesting user: ${userId}, isOwner: ${isOwner}`);
        return isOwner;
      }
    }
    
    console.log(`[PlaylistRepository] Playlist "${playlistName}" not found`);
    return false;
  } catch (error) {
    console.error('[PlaylistRepository] Error checking playlist management rights:', error);
    return false;
  }
};
